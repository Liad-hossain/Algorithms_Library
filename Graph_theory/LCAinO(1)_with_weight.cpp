...
namespace LCA{
    int sum[N],st[N],en[N],id[N];
    int lg[N],par[N],a[N],dp[LOG][N];
    vector<int> weight[N], g[N];
    int n, r, Time, cur;
    ...
    void build(){
        ...
        for (int l = 0; l < LOG - 1; l++){
            for (int i = 1; i <= n; i++){
                dp[l + 1][i] = dp[l][i];
                if ((1 << l) + i <= n && sum[dp[l][i + (1 << l)]] < sum[dp[l][i]])
                    dp[l + 1][i] = dp[l][i + (1 << l)];
            }
        }
    }
