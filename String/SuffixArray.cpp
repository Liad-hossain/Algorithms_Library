...
using namespace std;
const int N = 3e5 + 9;
void induced_sort(const vector<int> &vec, int
val_range,
...
vector<int> SA_IS(const vector<int> &vec,
int val_range) {
...
long long numofdifsubstr(){
long long ans=n-sa[0];
for(int i=1;i<n;i++)
ans+=(n-sa[i])-get_lcp(sa[i-1],sa[i]);
return ans;
}
// for ub (>t)
