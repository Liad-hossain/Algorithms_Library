...
{
  long long v1=query(x2,y2,0)*x2*y2+query(x2,y2,1)*x2+
          query(x2,y2,2)*y2+query(x2,y2,3);
  long long v2=query(x2,y1-1,0)*x2*(y1-1)+ query(x2,y1-1,1)*x2+
          query(x2,y1-1,2)*(y1-1)+query(x2,y1-1,3);
  long long v3=query(x1-1,y2,0)*(x1-1)*y2+query(x1-1,y2,1)*(x1-1)+
          query(x1-1,y2,2)*y2+query(x1-1,y2,3);
  long long v4=query(x1-1,y1-1,0)*(x1-1)*(y1-1)+
